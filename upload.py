...
def upload_to_cloudinary(image_bytes):
    ...
    return cloudinary.uploader.upload(
        ...
        format="png",           # 👈 enforce file format
        folder="bg-processed"
    )
...
def process_and_upload_image(image_data, mode):
    try:
        ...
        output_io.seek(0)
        return {"success": True, "image_url": upload_to_cloudinary(output_io)["secure_url"]}
...
def replace_background():
    ...
    try:
        ...
        return jsonify({
            ...
            "image_url": upload_to_cloudinary(output_io)["secure_url"]
        }), 200
...
def upload_image():
    ...
    if result["success"]:
        return jsonify({
            "message": f"Image processed in {mode} mode",
